...
export class Breadcrumbs implements Integration {
  ...
  private _instrumentRequest(): void {
    ...
    fill(sdk, 'request', originalRequest => {
      const wrappedRequest: typeof sdk['request'] = (requestOptions: any) => {
        ...
        if (dsn) {
          ...
          if (filterUrl && url.includes(filterUrl)) {
            ...
            return originalRequest.call(sdk, requestOptions)
          }
        }
  ...
  private _instrumentNavigation(): void {
    function handleNavigate(originalNavigate: Function) {
      return (options: { url?: string, delta?: number } = {} as any) => {
        ...
        return originalNavigate.call(sdk, options)
      }
    }
  ...
  private _instrumentApi(): void {
    Object.keys(sdk)
      .filter(api => {
        return typeof (sdk as any)[api] === 'function'
      })
      .forEach(api => {
        fill(sdk, api, original => {
          return (...args: any[]) => {
  ...
  private _instrumentLifecycle(): void {
    (['onAppShow', 'onAppHide'] as Array<keyof typeof sdk>).forEach(api => {
      if (!sdk[api]) {
        return
      }

      (sdk[api] as any)((res: any) => {
        Breadcrumbs.addBreadcrumb({
...
function addSentryBreadcrumb(serializedData: any): void {
  ...
  try {
    const event: { [key: string]: any } = isString(serializedData)
