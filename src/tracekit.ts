...
TraceKit._report = (function reportModuleWrapper() {
  ...
  let _oldOnerrorHandler: any, _onErrorHandlerInstalled: any
  ...
  function _traceKitWindowOnUnhandledRejection(e: any) {
    ...
    const stack = TraceKit._computeStackTrace(err)
  ...
  function processLastException() {
    const _lastExceptionStack = lastExceptionStack,
  ...
  function _report(ex: any) {
    ...
    const stack = TraceKit._computeStackTrace(ex)
...
TraceKit._computeStackTrace = (function _computeStackTraceWrapper() {
  ...
  function _computeStackTraceFromStacktraceProp(ex: any) {
    ...
    let opera10Regex = / line (\d+).*script (?:in )?(\S+)(?:: in function (\S+))?$/i,
  ...
  function _computeStackTraceFromOperaMultiLineMessage(ex: any) {
    ...
    let lineRE1 = /^\s*Line (\d+) of linked script ((?:file|https?|blob)\S+)(?:: in function (\S+))?\s*$/i,
    ...
    for (let line = 2; line < lines.length; line += 2) {
      ...
      if ((parts = lineRE1.exec(lines[line]))) {
        ...
        const url = getLocationHref().replace(/#.*$/, '')
        item = {
          url: url,
          func: '',
          args: [],
